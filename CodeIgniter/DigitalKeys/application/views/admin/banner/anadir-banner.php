...
<head>
	<meta charset="UTF-8">
	<title>Añadir Banner -  DigitalKey</title>
	...
	<script type="text/javascript">
		function validarModificacion(){
			...
			valor = document.getElementById("alt").value;
			if(valor.length == 0){
				alert("Faltan el alt");
				return false;
			}
...
<div class="cuerpo mt-5">
	<div class="mod container text-center mt-5">
		<form name="formularioAñadir"class="user" method="post" action="<?php echo base_url() . "Admin/anadirBanner"; ?>" onsubmit="return validarModificacion()" enctype="multipart/form-data" >
			<div class="d-sm-flex align-items-center justify-content-center mb-4">
				<h1 class="h3 mb-0 text-gray-800">Añadir Banner</h1>
			</div>
			...
			<div class="form-group">
				<input type="text" class="form-control-2 form-control-user" id="alt" name="alt" placeholder="Texto Alternativo">
			</div>

			<div class="form-group">
				Id Promoción
				<select class="form-control-2" id="promoid" name="promoid" >
					<option value="">Selecciona una promocion para añadirla a un banner</option>
					<?php
					$longitud = count($productos);
					for ($k = 0; $k < $longitud; $k++){
						echo '<option value="'.$productos[$k]["nombre"].'">Id: '.$productos[$k]["id"].', promo: '.$productos[$k]["nombre"].'</option><br>';
					}
					?>
				</select>
			</div>
			...
			<div class="d-sm-flex align-items-center justify-content-center mt-5 mb-4">
				<a class="btn-carrito verde-perfil btn-icon-split btn-sm">
					...
					<span class="text">
						<input type="submit" class="btn btn-user" id="enviar" name="enviar" value="Guardar Banner">
					</span>
				</a>
			</div>
		</form>
