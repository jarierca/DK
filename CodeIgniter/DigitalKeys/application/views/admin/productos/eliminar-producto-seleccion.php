...
<head>
	<meta charset="UTF-8">
	<title>Eliminar Producto -  DigitalKey</title>
	...
	<script type="text/javascript">
		function validarModificacion(){
			...
			valor = document.getElementById("productoBorrar").value;
			if(valor.length == "0"){
...
<div class="cuerpo mt-5">
	<div class="mod container text-center mt-5">
		<form name="formularioAñadir"class="user" method="post" action="<?php echo base_url() . "Admin/eliminarProducto"; ?>" onsubmit="return validarModificacion()" enctype="multipart/form-data" >
			<div class="d-sm-flex align-items-center justify-content-center mb-4">
				<h1 class="h3 mb-0 text-gray-800">Eliminar Producto</h1>
			</div>

			<select class="form-control-2" id="productoBorrar" name="productoBorrar" >
				<option value="">Selecciona el producto para borrar sus datos</option>
				<?php
				$longitud = count($producto);
				for ($j = 0; $j < $longitud; $j++){
					echo '<option value="'.$producto[$j]["id"].'">Id: '.$producto[$j]["id"].', nombre: '.$producto[$j]["nombre"].', stock: '.$producto[$j]["stock"].'</option><br>';
				}
				?>
			</select>
