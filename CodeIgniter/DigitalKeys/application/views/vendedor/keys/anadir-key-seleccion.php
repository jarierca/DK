...
<head>
	<meta charset="UTF-8">
	<title>Modificar Key -  DigitalKey</title>
	...
	<script type="text/javascript">
		function validarModificacion(){
			...
			valor = document.getElementById("productoMod").value;
			if(valor.length == "0"){
...
<div class="cuerpo mt-5">
	<div class="mod container text-center mt-5">
		<form name="formularioAñadir"class="user" method="post" action="<?php echo base_url() . "Vendedor/anadirKeyVista"; ?>" onsubmit="return validarModiciacion()" enctype="multipart/form-data" >
			<div class="d-sm-flex align-items-center justify-content-center mb-4">
				<h1 class="h3 mb-0 text-gray-800">Añadir Key</h1>
			</div>

			<select class="form-control-2" id="proSelecc" name="proSelecc" >
				<option value="">Selecciona el producto para añadir una key</option>
				<?php
				$longitud = count($productos);
				for ($j = 0; $j < $longitud; $j++){
					echo '<option value="'.$productos[$j]["id"].'">Id: '.$productos[$j]["id"].', nombre: '.$productos[$j]["nombre"].'</option><br>';
				}
				?>
			</select>
			<div class="d-sm-flex align-items-center justify-content-center mt-5 mb-4">
				<a class="btn-carrito verde-perfil btn-icon-split btn-sm">
					<span class="text">
						<input type="submit" class="btn btn-user" id="enviar" name="enviar" value="Seleccionar Producto">
					</span>
				</a>
			</div>
		</form>
