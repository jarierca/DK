...
<head>
	...
	<script type="text/javascript">
		function validarModificacion(){
			...
			valor = document.getElementById("adminMod").value;
			if(valor.length == "0"){
				alert("Debes seleccionar un admin");
				return false;
			}
			return true;
		}
...
<div class="cuerpo mt-5">
	<div class="mod container text-center mt-5">
		<form name="formularioAñadir"class="user" method="post" action="<?php echo base_url() . "Admin/modificarAdminVistaDatos"; ?>" onsubmit="return validarModiciacion()" enctype="multipart/form-data" >
			<div class="d-sm-flex align-items-center justify-content-center mb-4">
				<h1 class="h3 mb-0 text-gray-800">Modificar Admin</h1>
			</div>

			<select class="form-control-2" id="adminMod" name="adminMod" >
				<option value="">Selecciona un admin para modificar sus datos</option>
				<?php
				$longitud = count($admin);
				for ($j = 0; $j < $longitud; $j++){
					echo '<option value="'.$admin[$j]["id"].'">Id: '.$admin[$j]["id"].', nick: '.$this->cifrado->superdescifrar($admin[$j]["nick"]).', email: '.$this->cifrado->superdescifrar($admin[$j]["mail"]).'</option><br>';
				}
				?>
			</select>
			<div class="d-sm-flex align-items-center justify-content-center mt-5 mb-4">
				<a class="btn-carrito verde-perfil btn-icon-split btn-sm">
					<span class="text">
						<input type="submit" class="btn btn-user" id="enviar" name="enviar" value="Seleccionar Admin">
					</span>
				</a>
			</div>
		</form>
