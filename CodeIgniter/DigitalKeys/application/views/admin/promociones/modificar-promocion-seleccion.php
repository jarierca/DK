...
<head>
	...
	<script type="text/javascript">
		function validarModificacion(){
			...
			valor = document.getElementById("promoMod").value;
			if(valor.length == "0"){
...
<div class="cuerpo mt-5">
	<div class="mod container text-center mt-5">
		<form name="formularioAñadir"class="user" method="post" action="<?php echo base_url() . "Admin/modificarPromocionesVistaDatos"; ?>" onsubmit="return validarModificacion()" enctype="multipart/form-data" >
			<div class="d-sm-flex align-items-center justify-content-center mb-4">
				<h1 class="h3 mb-0 text-gray-800">Modificar Promoción</h1>
			</div>

			<select class="form-control-2" id="promoMod" name="promoMod" >
				<option value="">Selecciona una promoción para modificar sus datos</option>
				<?php
				$longitud = count($productos);
				for ($j = 0; $j < $longitud; $j++){
					echo '<option value="'.$productos[$j]["id"].'">Id: '.$productos[$j]["id"].', promo: '.$productos[$j]["nombre"].'</option><br>';
				}
				?>
			</select>
