...
<head>
	...
	<script type="text/javascript">
		function validarModificacion(){
			...
			valor = document.getElementById("bannerMod").value;
			if(valor.length == "0"){
...
<div class="cuerpo mt-5">
	<div class="mod container text-center mt-5">
		<form name="formularioAñadir"class="user" method="post" action="<?php echo base_url() . "Admin/modificarBannerVistaDatos"; ?>" onsubmit="return validarModificacion()" enctype="multipart/form-data" >
			<div class="d-sm-flex align-items-center justify-content-center mb-4">
				<h1 class="h3 mb-0 text-gray-800">Modificar Banner</h1>
			</div>

			<select class="form-control-2" id="bannerMod" name="bannerMod" >
				<option value="">Selecciona el banner para modificar sus datos</option>
				<?php
				$longitud = count($productos);
				for ($j = 0; $j < $longitud; $j++){
					$estado = "";
					if ($productos[$j]["activado"] == "1"){
						$estado = "Activado";
					}else{
						$estado = "Desctivado";
					}
					echo '<option value="'.$productos[$j]["id"].'">Id: '.$productos[$j]["id"].', promo: '.$productos[$j]["promocion"].', estado: '.$estado.'</option><br>';
				}
				?>
			</select>
			<div class="d-sm-flex align-items-center justify-content-center mt-5 mb-4">
				<a class="btn-carrito verde-perfil btn-icon-split btn-sm">
					<span class="text">
						<input type="submit" class="btn btn-user" id="enviar" name="enviar" value="Seleccionar Banner">
					</span>
				</a>
			</div>
		</form>
